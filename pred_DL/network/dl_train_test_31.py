...
def emb_seq_BE(seq, aa_dict, kmer):
   seq_emb = np.array([np.array([aa_dict[seq[i + k]] for k in range(kmer)]).reshape([4 * kmer]) for i in range(len(seq) - kmer + 1)])
   return seq_emb
  
def emb_seq_w2v(seq, w2v_model, kmer):
...
class pv_data_sets():
    ...
    def __getitem__(self, idx):
        ...
        elif 'W2V' in self.encode_method:
...
class train_test_process():
    def __init__(self, out_path, loss_type = "balanced", tra_batch_size = 128, val_batch_size = 128, test_batch_size = 32, features = 64, lr = 0.001, n_epoch = 50, early_stop = 25, thresh = 0.5): #lr = 0.00001,
        self.out_path = out_path
        self.tra_batch_size = tra_batch_size
        self.val_batch_size = val_batch_size
        self.features = features #temporaty size
        self.lr = lr
    ...
    def training_testing(self, train_data_sets, val_data_sets, test_data_sets, deep_method, encode_method, seqwin, kmer, w2v_model, w2v_feature):
        ...
        if "W2V" in encode_method :
              self.features = w2v_feature
        else:
              self.features = 4*kmer  #one hot encoding size
...
if __name__=="__main__":      
    ...
    parser.add_argument('--deep_encode', help='Path')
    parser.add_argument('--fold', type=int, help='Value')
    parser.add_argument('--seqwin', type=int, help='Value')
    parser.add_argument('--kmer', type=int, help='Value')
    parser.add_argument('--size', type=int, help='Value')
    parser.add_argument('--epochs', type=int, help='Value')
    parser.add_argument('--sg', type=int, help='Value')
    parser.add_argument('--window', type=int, help='Value')

    path = parser.parse_args().intrain
    test_path = parser.parse_args().intest
    out_path_ori = parser.parse_args().outpath
    loss_type = parser.parse_args().losstype
    w2v_model_ori = parser.parse_args().w2vmodel
    deep_method = parser.parse_args().deeplearn
        
    deep_encode = parser.parse_args().deep_encode.strip().split(',')
    
    kfold = parser.parse_args().fold
    seqwin = parser.parse_args().seqwin
    kmer = parser.parse_args().kmer
    w2v_feature = parser.parse_args().size
    ...
    for encode_method in deep_encode:
        ...
        if 'W2V' in encode_method:
           kmer = int(encode_method[4])
           w2v_model = w2v_model_ori.replace("kmer", str(kmer))
           w2v_model = word2vec.Word2Vec.load(w2v_model)

           #out_path =  out_path_ori + '/' + deep_method + '/W2V_' + str(kmer) + '_' + str(w2v_feature) + '_' + str(epochs) + '_' + str(window) + '_' + str(sg)
           out_path = out_path_ori + '/' + deep_method + '/W2V_' + str(kmer) 
           os.makedirs(out_path, exist_ok=True)
        else:
           w2v_model = []
           kmer = 1 #default value
           out_path =  out_path_ori + '/' + deep_method + '/' + encode_method 
           os.makedirs(out_path, exist_ok=True) #+ '_' + str(kmer)
        ...
        for i in range(1, kfold+1):
            ...
            net.training_testing(train_dataset, val_dataset, test_dataset, deep_method, encode_method, seqwin, kmer, w2v_model, w2v_feature)
