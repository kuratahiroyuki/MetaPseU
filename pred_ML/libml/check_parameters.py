...
def check_acc_arguments(args):
    ...
    kmer = myKmer[args.encode] ### method
    myIndex = []
    myProperty = {}
    dataFile = ''

    if myNum == 0:
        myIndex = myDictDefault[args.encode][args.type] ###
        dataFile = myDataFile[args.encode][args.type] ###
    else:
        if args.index:
            with open(args.index) as f:
                records = f.read().strip().split('\n')
                for i in records:
                    if i in myDict[args.encode][args.type]:
                        myIndex.append(i)
                    else:
                        print('Error: there is no "%s" in the index list.' % i)
                        sys.exit(1)
        if args.all_index:
            myIndex = myDict[args.encode][args.type] ###
...
def check_Pse_arguments(args, fastas):
    ...
    for i in fastas:
        if len(i[0]) < fastaMinLength: ### i[1]=> i[0]
