...
class AUCMeter():
    ...
    def auc(self):
        ...
        precision, recall, thresholds_pr = precision_recall_curve(self.targets, self.scores)
        prauc = auc(x=recall, y=precision) 
        
        fpr, tpr, thresholds = roc_curve(self.targets, self.scores)
...
def eval_metrics(probs, targets, cal_AUC=True):
    ...
    if cal_AUC:
        if isinstance(probs, torch.Tensor) and isinstance(targets, torch.Tensor):
            fpr, tpr, thresholds = roc_curve(targets.detach().cpu().numpy(), probs.detach().cpu().numpy())

            precision, recall, thresholds_pr = precision_recall_curve(targets.detach().cpu().numpy(), probs.detach().cpu().numpy())
            
        elif isinstance(probs, np.ndarray) and isinstance(targets, np.ndarray):
            fpr, tpr, thresholds = roc_curve(targets, probs)
            precision, recall, thresholds_pr = precision_recall_curve(targets, probs)
        else:
